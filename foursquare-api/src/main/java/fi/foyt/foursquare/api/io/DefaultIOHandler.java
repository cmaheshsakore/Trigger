...
public class DefaultIOHandler extends IOHandler {
  ...
  public Response fetchData(String url, Method method) {
    ...
    try {
      ...
      try {
        // ICS is having issues with GET and HttpURLConnection.
        // The object returns get but the Foursquare endpoint believes it to be post.  Manually use
        // the signpost libs for GET for now - otherwise default back to HttpURLConnection
        if (method.name().equals("GET"))
        {
	        ...
	        {
	        	return new Response(new String(responseBody), 200, "");
	        }
        }
        else
        {
	        ...
	        if (code == 200) {
	          ...
	          response = new Response(readStream(inputStream), code, connection.getResponseMessage());
	        } else {
	          response =  new Response("", code, getMessageByCode(code));
	        }
